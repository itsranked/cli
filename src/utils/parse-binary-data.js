 ...
 */
 function parseBinaryData(connection, buffer) {
    ...
    if (messageType === 0x36) { // Pre-init response
    ...
    if (messageType === 0x61) { // Initial Setup
    ...
    if (messageType === 0x6c) { // Leaderboard
...
//        const totalPlayers = buffer.subarray(offset, offset + 2).readUIntBE(0, 2);
        ...
        do {
            ...
            const score = Math.floor(15 * (fpsls[snakeBodyParts] + snakeLastBodyPartFullness / fmlts[snakeBodyParts] - 1) - 5) / 1;
